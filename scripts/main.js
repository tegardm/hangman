...
const lastAnswer = document.querySelector('.answer-cont .answer')
let clueWord = null;
...
const getRandomWord = () => {
	if (words.length > 0) {
		...
		clueWord = words[current]
		lastAnswer.textContent = clueWord
...
const reduceGuesses = () => {
	...
	if (diffButton.textContent == "Hard") {
		guessesSpan.innerText = `${guesses}/7`;
	} else {
